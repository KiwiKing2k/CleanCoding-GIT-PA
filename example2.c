...
int *insedg(int nr_of_vertices, int nr_of_edges, GPH *graph) {
    /// sa il tunete dumnezeu pe ala de a scris asta
    int src, dest, i;
    printf("adauga %d muchii (de la 1 la %d)\n", nr_of_edges, nr_of_vertices);
    for (i = 0; i < nr_of_edges; i++) {
        scanf("%d%d", &src, &dest);
        ///bagam niste modificari ca nu merge nici cu debug prostia asta
...
int main() {
    ...
    return 0;
    ///prin puterea zeilor nordici am reusit sa fac chestia asta sa mearga nu incercati cu altceva ca sigur bubuie
}
